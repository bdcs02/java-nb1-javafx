...
public class SoccerController {
    ...
    public TableColumn playerRateColumn;
    @FXML
    ...
    @FXML
    private GridPane streamGrid;
    @FXML
    private Label streamGridLabel;


    ChoiceBox DataMoreA2CB = new ChoiceBox();

    ChoiceBox StreamChoice = new ChoiceBox();
    ...
    public void ElemekTörlése() {
        ...
        streamGrid.setVisible(false);
        streamGrid.setManaged(false);

        streamGridLabel.setManaged(false);
        streamGridLabel.setVisible(false);

        StreamChoice.setVisible(false);
        StreamChoice.setManaged(false);

        openPos.setVisible(false);
        openPos.setManaged(false);

        CloseTradeIdTextField.setVisible(false);
        CloseTradeIdTextField.setManaged(false);
...
// 10 legközelebbi szomszéd:
    ...
    public void menuStream(ActionEvent actionEvent) {
        ...
        streamGridLabel.setVisible(true);
        streamGridLabel.setManaged(true);
        streamGridLabel.setText("Válasszon ki egy elemet!");

        streamGrid.setVisible(true);
        streamGrid.setManaged(true);

        String st[] = {"10 betűnél kisebb focisták", "700nál nagyobb értékszám", "Magyar és 1995 után született", "Külföldi és 500 feletti"};

        // create a choiceBox
        StreamChoice = new ChoiceBox(FXCollections.observableArrayList(st));

        streamGrid.getChildren().add(StreamChoice);
        StreamChoice.setVisible(true);
        StreamChoice.setManaged(true);

        // add a listener
        StreamChoice.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue ov, Number value, Number new_value) {
                ...
                if (new_value != null) {
                    ...
                    switch (st[new_value.intValue()]) {
                        case "10 betűnél kisebb focisták": {
                            Session session = SoccerApplication.getSession();
                            Transaction t = session.beginTransaction();
                            List<SoccersModel> lista = session.createQuery("FROM SoccersModel ORDER BY id ")
                                    .list();
                            StringBuilder allPlayersText = new StringBuilder();
                            lista.stream()
                                    .filter(p -> (p.getFirstName().length() + p.getLastName().length()) < 10)
                                    .forEach(a -> allPlayersText.append(a.getFirstName()).append(" ").append(a.getLastName()).append("\n"));

                            streamGridLabel.setText(allPlayersText.toString());
                        ...
                        case "700nál nagyobb értékszám": {
                            ...
                            StringBuilder allPlayersText = new StringBuilder();
                            lista.stream()
                                    .filter(p -> Integer.parseInt(p.getPlayerRate()) > 700)
                                    .forEach(a -> allPlayersText.append(a.getFirstName()).append(" ").append(a.getLastName()).append(" ").append(a.getPlayerRate()).append("\n"));
                        ...
                        case "Magyar és 1995 után született": {
                            Session session = SoccerApplication.getSession();
                            Transaction t = session.beginTransaction();
                            List<SoccersModel> lista = session.createQuery("FROM SoccersModel ORDER BY id ")
                                    .list();
                            StringBuilder allPlayersText = new StringBuilder();
                            Predicate<SoccersModel> isHungarianPredicate = p -> Integer.parseInt(p.getIsHungarian()) == -1;
                            Predicate<SoccersModel> bornAfter1995Predicate = p -> {
                                String[] birthDateParts = p.getBirthDate().split("-");
                                int year = Integer.parseInt(birthDateParts[0]);
                                return year > 1995;
                            };

                            lista.stream()
                                    .filter(isHungarianPredicate.and(bornAfter1995Predicate))
                                    .forEach(a -> allPlayersText.append(a.getFirstName()).append(" ").append(a.getLastName()).append("\n"));
                        ...
                        case "Külföldi és 500 feletti": {
                            Session session = SoccerApplication.getSession();
                            Transaction t = session.beginTransaction();
                            List<SoccersModel> lista = session.createQuery("FROM SoccersModel ORDER BY id ")
                                    .list();
                            StringBuilder allPlayersText = new StringBuilder();
                            Predicate<SoccersModel> isForeignPredicate = p -> Integer.parseInt(p.getIsForeign()) == -1;
                            Predicate<SoccersModel> playerRatingPredicate = p -> Integer.parseInt(p.getPlayerRate()) > 500;


                            lista.stream()
                                    .filter(isForeignPredicate.and(playerRatingPredicate))
                                    .forEach(a -> allPlayersText.append(a.getFirstName()).append(" ").append(a.getLastName()).append(" ").append(a.getPlayerRate()).append("\n"));
            ...
            try {
                ...
                while (true) {
                    ...
                    for (com.oanda.v20.pricing.ClientPrice price : resp.getPrices()) {
                        ...
                        Platform.runLater(() -> {
                            pricesNow.setText("Price: " + price.getBids());
                        });
                    }
                    since = resp.getTime();
                    Thread.sleep(1000);
    ...
    @FXML
    private Label openPos;
    @FXML
    public void menuOpenPos(ActionEvent event) {
        ElemekTörlése();
        openPos.setManaged(true);
        openPos.setVisible(true);
        com.oanda.v20.Context ctx = new com.oanda.v20.ContextBuilder(Config.URL)
        ...
        com.oanda.v20.primitives.InstrumentName instrument = new com.oanda.v20.primitives.InstrumentName("NZD_USD");
        ...
        try {
            ...
            for (com.oanda.v20.trade.Trade trade : trades) {
                System.out.println("Instrument: " + trade.getInstrument());
                String labelText = "tradeId: " + response.getOrderFillTransaction().getId() + "\n" +
                        "Instrument: " + trade.getInstrument() + "\n" +
                        "Successful" + "\n";

                openPos.setText(labelText);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    @FXML
    private TextField CloseTradeIdTextField;
    @FXML
