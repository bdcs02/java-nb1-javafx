...
public class SoccerController {
    ...
    public void ElemekTörlése() {
        ...
        playerRateColumn.setVisible(false);
    }
                    ...
                    switch (st[new_value.intValue()]) {
                        ...
                        case "Magyar és 1995 után született": {
                            ...
                            List<SoccersModel> lista = session.createQuery("FROM SoccersModel ORDER BY id ")
                                    .list();
                            StringBuilder allPlayersText = new StringBuilder();
                            Predicate<SoccersModel> isHungarianPredicate = p -> {
                                String isHungarianText = p.getIsHungarian();
                                return isHungarianText != null && !isHungarianText.isEmpty() && Integer.parseInt(isHungarianText) == -1;
                            };
                        ...
                        case "Külföldi és 500 feletti": {
                            Session session = SoccerApplication.getSession();
                            Transaction t = session.beginTransaction();
                            List<SoccersModel> lista = session.createQuery("FROM SoccersModel ORDER BY id ").list();
                            StringBuilder allPlayersText = new StringBuilder();

                            Predicate<SoccersModel> isForeignPredicate = p -> {
                                String isForeignText = p.getIsForeign();
                                return isForeignText != null && !isForeignText.isEmpty() && Integer.parseInt(isForeignText) == -1;
                            };

                            Predicate<SoccersModel> playerRatingPredicate = p -> {
                                String playerRateText = p.getPlayerRate();
                                return playerRateText != null && !playerRateText.isEmpty() && Integer.parseInt(playerRateText) > 500;
                            };
